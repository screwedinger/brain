...
function isTrackableUrl(url) { return !!url && !/^(chrome|edge|about|devtools|chrome-extension|view-source):/i.test(url); }
...
async function all(store, limit = 100) {
  ...
  return rows.sort((a,b) => (b.startedAt || b.timestamp || b.lastSeenAt || 0) - (a.startedAt || a.timestamp || a.lastSeenAt || 0)).slice(0, limit);
}
...
function lockTab(tabId, work) {
  ...
  tabLocks.set(tabId, next.finally(() => { if (tabLocks.get(tabId) === next) tabLocks.delete(tabId); }));
  return next;
}
...
async function closeAllActiveVisits() {
  const activeRows = await all(ACTIVE_STORE, 10000);
  for (const active of activeRows) {
    const visit = await get(VISIT_STORE, active.visitId);
    if (visit && !visit.endedAt) {
      const now = Date.now();
      visit.endedAt = now;
      visit.durationMs = Math.max(0, now - visit.startedAt);
      await put(VISIT_STORE, visit);
    }
    await tx(ACTIVE_STORE, 'readwrite', s => s.delete(active.tabId));
  }
  await tx(STATE_STORE, 'readwrite', s => s.delete('focus'));
}
...
async function startVisit(tabId, url, title = '', windowId, transitionType = 'link') {
  ...
  const page = {
    id: existing?.id || uuid(), url: clean, title: title || existing?.title || clean,
    description: existing?.description || '', headings: existing?.headings || [], contentText: existing?.contentText || '',
    firstSeenAt: existing?.firstSeenAt || now, lastSeenAt: now, visitCount: (existing?.visitCount || 0) + 1,
    contentHash: existing?.contentHash
  };
  await put(PAGE_STORE, page);
  const visit = { id: uuid(), tabId, windowId, pageId: page.id, url: clean, title: page.title, startedAt: now, source: 'navigation', transitionType };
  await put(VISIT_STORE, visit);
...
async function captureTab(tab, { startVisitIfActive = true, transitionType = 'link' } = {}) {
  ...
  if (current) {
    ...
    current.lastSeenAt = Math.max(current.lastSeenAt || 0, now);
    await put(PAGE_STORE, current);
  } else {
    await put(PAGE_STORE, { id: uuid(), url: clean, title: tab.title || clean, description: '', headings: [], contentText: '', firstSeenAt: now, lastSeenAt: now, visitCount: 0 });
  }
...
async function importHistory() {
  ...
  for (const item of items) {
    ...
    const page = {
      id: existing?.id || uuid(), url, title: item.title || existing?.title || url,
      description: existing?.description || '', headings: existing?.headings || [], contentText: existing?.contentText || '',
      ...
      visitCount: Math.max(existing?.visitCount || 0, item.visitCount || relevant.length || 1), contentHash: existing?.contentHash
    };
    await put(PAGE_STORE, page);
    for (const visit of relevant) {
      const id = `history:${visit.id}`;
      if (await get(VISIT_STORE, id)) continue;
      await put(VISIT_STORE, { id, tabId: null, windowId: null, pageId: page.id, url, title: page.title, startedAt: visit.visitTime, endedAt: visit.visitTime, durationMs: 0, source: 'history', transitionType: visit.transition });
      importedVisits++;
    }
  }
...
async function boot() {
  await closeAllActiveVisits();
  await importHistory().catch(() => {});
  await captureOpenTabs().catch(() => {});
}

chrome.webNavigation.onCommitted.addListener(async d => {
  if (d.frameId === 0) await event('navigation', { tabId: d.tabId, windowId: d.windowId, url: safeUrl(d.url), transitionType: d.transitionType });
});
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => lockTab(tabId, async () => {
  ...
  if (changeInfo.url) await event('tab_updated', { tabId, windowId: tab.windowId, url, title: tab.title || '' });
  if (changeInfo.status === 'complete' || changeInfo.url) {
    const activeTabs = await chrome.tabs.query({ active: true, lastFocusedWindow: true }).catch(() => []);
    await captureTab(tab, { startVisitIfActive: activeTabs[0]?.id === tabId, transitionType: changeInfo.url ? 'link' : 'reload' });
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message?.type === 'PAGE_CAPTURE') {
    (async () => {
      ...
      await put(PAGE_STORE, {
        ...
        visitCount: existing?.visitCount || 0, contentHash: data.contentHash || existing?.contentHash
      });
      return { ok: true };
  ...
  if (message?.type === 'GET_RECENT_MEMORY') {
    Promise.all([count(EVENT_STORE), count(PAGE_STORE), count(VISIT_STORE), all(PAGE_STORE,50), all(VISIT_STORE,100), all(EVENT_STORE,150), get(STATE_STORE,'focus')])
      .then(([eventsCount,pagesCount,visitsCount,pages,visits,events,focus]) => sendResponse({ok:true,eventsCount,pagesCount,visitsCount,pages,visits,events,focus}))
